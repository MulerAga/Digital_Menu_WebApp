...
export default function FloatingPromo() {
  ...
  const expectedPath = branchSlug ? `/${slug}/branches/${branchSlug}` : `/${slug}`;
  const isMenuPage = location.pathname === expectedPath || location.pathname === `${expectedPath}/`;

  const isCustomer = !user || user.role === 'customer';

  useEffect(() => {
    if (!restaurantSlug) return;
    menuAPI.getPublicPromotions(restaurantSlug, branchSlug || null)
      .then((res) => {
        if (res.data.length) {
          setPromos(res.data);
      ...
      <div className="card shadow-2xl border-2 border-primary-200 dark:border-primary-800 overflow-hidden">
        <div className="bg-gradient-to-r from-primary-500 to-orange-400 px-4 py-2 flex items-center justify-between">
          <div className="flex items-center gap-2 text-white">
            ...
            <span className="text-sm font-bold">
              {promo.discount_percent ? `${promo.discount_percent}% OFF!` : promo.is_featured ? 'Featured' : 'Recommended'}
            </span>
          </div>
          <button
        ...
        <div className="p-3 flex gap-3 items-center">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{promo.category_name}</p>
            <div className="flex items-center gap-2 mt-1">
              {discountedPrice ? (
                <>
                  <span className="text-primary-600 font-bold text-sm">${discountedPrice}</span>
                  <span className="text-gray-400 line-through text-xs">${Number(promo.price).toFixed(2)}</span>
                </>
              ) : (
                <span className="text-primary-600 font-bold text-sm">${Number(promo.price).toFixed(2)}</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="px-3 pb-3 flex gap-2">
          <button onClick={handleOrderNow} className="flex-1 btn-primary text-xs py-1.5">
            {buttonLabel}
          </button>
          {promos.length > 1 && (
            <button
